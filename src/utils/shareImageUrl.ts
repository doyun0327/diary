...
export async function compressDataUrlForShare(
  ...
  try {
    ...
    return await resizeToJpeg(trimmed);
  } catch {
    return src.trim();
  }
}

function resizeToJpeg(src: string): Promise<string> {
  return new Promise((resolve) => {
    ...
    img.onload = () => {
      ...
      const scale = Math.min(1, SHARE_MAX_EDGE / Math.max(w0, h0));
      ...
      resolve(canvas.toDataURL('image/jpeg', SHARE_JPEG_QUALITY));
    };
